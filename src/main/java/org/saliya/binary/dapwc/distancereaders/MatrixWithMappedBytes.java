...
public class MatrixWithMappedBytes {
    public double getDistance(int globalRow, int globalCol){
        throw new UnsupportedOperationException();
    }

    public static MatrixWithMappedBytes readRowRange(String fname, Range rows, int globalColCount, int dataTypeSize, ByteOrder
        ...
        if (mmap) {
            try (FileChannel fc = (FileChannel) Files.newByteChannel(Paths.get(fname), StandardOpenOption.READ)) {
                ...
                return new MatrixWithMappedBytes(){
                    @Override
                    public double getDistance(int globalRow, int globalCol) {
