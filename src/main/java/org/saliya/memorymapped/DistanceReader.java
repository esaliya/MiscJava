...
public interface DistanceReader {
    ...
    public static DistanceReader readRowRange(String fname, Range rows, int globalColCount, int dataTypeSize, ByteOrder
        ...
        if (mmap) {
            try (FileChannel fc = (FileChannel) Files.newByteChannel(Paths.get(fname), StandardOpenOption.READ)) {
                ...
                return (globalRow, globalCol) -> {
                    ...
                    return mappedBytes.getShort(position * 2) / (Short.MAX_VALUE * 1.0); // pos*2 is the byte position
                };
            } catch (IOException e) {
                e.printStackTrace();
            }
